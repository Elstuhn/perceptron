...
class NeuralNetwork:
    ...
    def backprop(self, yhat, x, y, lr, bias):
        ...
        print(f"W1: {self.inputlayer[0].weight} W2: {self.inputlayer[1].weight} bias: {self.weightbias}")
    ...
    def changeNeurons(self, values : tensor):
        for i in range(len(self.inputlayer)):
            self.inputlayer[i].value = values[i]
...
def XOR(x : tensor):
    NOT = NeuralNetwork(1, [-1])
    AND = NeuralNetwork(2, [1, 1])
    OR = NeuralNetwork(2, [1, 1])
    print(NOT.test(np.array([0]), 0.5))
    print(AND.test(np.array([0, 1]), -1.5))
    print(OR.test(np.array([1, 0]), -0.5))
    inputs = np.array(x)
    op1 = AND.test(inputs, -1.5)
    op2 = NOT.test(np.array([op1]), 0.5)
    op3 = OR.test(inputs, -0.5)
    finalop = AND.test(np.array([op2, op3]), -1.5)
    return finalop
